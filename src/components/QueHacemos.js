...
export default function QueHacemos() {
  return (
    <section
      ...
      className="border-y border-border bg-paperSoft py-section-y"
    >
      <Container>
        <div className="grid grid-cols-12 gap-x-4 md:gap-x-8 gap-y-10 items-end mb-12">
          <p className="col-span-12 md:col-span-3 text-[10px] uppercase tracking-[0.24em] text-inkSoft">
            · Qué hacemos
          </p>
          <h2
            id="que-hacemos"
            className="col-span-12 md:col-span-9 font-display font-medium text-2xl md:text-3xl tracking-tighter leading-tight text-balance"
          >
        ...
        <ul className="grid grid-cols-1 md:grid-cols-3 gap-px bg-border border border-border">
          {blocks.map((b) => (
            <li
              key={b.n}
              className="bg-paper p-8 md:p-10 flex flex-col gap-4 hover:bg-paperSoft transition-colors"
            >
